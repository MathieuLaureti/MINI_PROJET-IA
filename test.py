...
default_name = f"{font_name}_{data_size}_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
...
# Run training commands for LSTM
def run_training_lstm():
    ...
    try:
        ...
        # Create shapetable file for LSTM
        print("Creating shapetable file for LSTM...")
        ...
        # Run LSTM training (using lstmtraining)
        print("Running lstmtraining...")
        lstmtraining_command = f"lstmtraining --training_text={LANGUAGE}_training_text.txt --train_listfile={' '.join(tr_files)} --model_output={LANGUAGE}.model"
        subprocess.run(lstmtraining_command, shell=True)
        
        # Run cntraining for LSTM
        print("Running cntraining...")
        cntraining_command = f"cntraining {' '.join(tr_files)}"
        subprocess.run(cntraining_command, shell=True)
        
        # Combine the output files into a single traineddata file
